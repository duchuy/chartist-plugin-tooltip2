(function (root, factory) {
  ...
  (function (window, document, Chartist) {
    ...
    var ie = (ua.indexOf('MSIE ') >= 0) || (ua.indexOf('Trident/') >= 0) || (ua.indexOf('Edge/') >= 0);
    ...
    Chartist.plugins.tooltip2 = function (options) {
      ...
      return function tooltip(chart) {
        ...
        function init() {
          ...
          if (chart instanceof Chartist.Line) {
            chart.on('created', function () {
              ...
              chart.container.querySelector('svg').addEventListener('touchmove', prepareLineTooltip);
              chart.container.addEventListener('touchend', function (e) {
                var pointElement = chart.container.querySelector('.' + chart.options.classNames.point + '--hover');
                hideTooltip(pointElement);
              });

              // collect pointValues after created due to IE rounds the value of cx, x1 to 2 or 3 decimal points therefore fails coordinate comparison 
              var pointElements = chart.container.querySelectorAll('.' + chart.options.classNames.point);
              Array.prototype.forEach.call(pointElements, function (point) {
                if (options.pointDOMType == 'circle') {
                    pointValues.push(point.getAttribute('cx'));
                } else {
                    pointValues.push(point.getAttribute('x1'));
                }
              });
            });
          ...
          chart.container.addEventListener('touchstart', delegate(triggerSelector, function (e) {
            showTooltip(e.target);
          }));
          chart.container.addEventListener('mouseout', delegate(triggerSelector, function (e) {
            hideTooltip(e.target);
          }));
          chart.container.addEventListener('touchend', delegate(triggerSelector, function (e) {
            hideTooltip(e.target);
          }));
        ...
        function prepareLineTooltip(e) {
          ...
          var currentXPosition = (e.type == 'touchmove' ? e.changedTouches[0].pageX : e.pageX) - (boxData.left + (document.documentElement.scrollLeft || document.body.scrollLeft));
          var currentYPosition = (e.type == 'touchmove' ? e.changedTouches[0].pageY : e.pageY) - (boxData.top + (document.documentElement.scrollTop || document.body.scrollTop));
        ...
        function showTooltip(triggerElement) {
          ...
          if (ie) {
            seriesGroups = [triggerElement.parentNode];
          } else {
        ...
        function getTooltipPosition(relativeElement) {
          ...
          var left = boxData.left + scroll_X + options.offset.x - width / 2 + boxData.width / 2;
          var top = boxData.top + scroll_Y - height + options.offset.y;
          ...
          if (left + width > document.body.clientWidth) {
            ...
            left = boxData.left + scroll_X - options.offsetCollision.x;
